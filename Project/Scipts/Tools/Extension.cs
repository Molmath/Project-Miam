...
namespace Com.MathysMoles.Extension
{
    public static class Extension
    {
        ...
        #region Action
        public static bool Contain<T>(this Action<T> pAction, string pMethodName, out Action<T> pDelegate)
        {
            if (pAction == null)
            {
                pDelegate = null;
            ...
            foreach (Action<T> lMethod in pAction?.GetInvocationList())
            {
        ...
        public static bool Contain<T>(this Action<T> pAction,  out List<Action<T>> pDelegates, params Action<T>[] pNames)
        {
            if (pAction == null)
            {
                pDelegates = null;
            ...
            List<Action<T>> lDelegates = new();
            foreach (Action<T> pName in pNames)
            {
                foreach (Action<T> lMethod in pAction?.GetInvocationList())
                {
                    if (lMethod == pName)
                    {
                        lDelegates.Add(lMethod);
                    }
                }
            }

            if(lDelegates.Count > 0)
            {
                pDelegates = lDelegates;
                return true;
            }
            pDelegates = null;
            return false;
